...
import * as path from 'path';  // Import all members using namespace



/** Our keypair we used to create the on-chain Rust program */
const PROGRAM_KEYPAIR_PATH = path.join(path.resolve(__dirname, '../../src/program/target/deploy'), 'program-keypair.json');
...
async function main() {
    ...
    const airdropRequest = await connection.requestAirdrop(triggerKeypair.publicKey,
        LAMPORTS_PER_SOL,
    );
    ...
    await sendAndConfirmTransaction(
        connection, new Transaction().add(instruction),[triggerKeypair],
    );
}
